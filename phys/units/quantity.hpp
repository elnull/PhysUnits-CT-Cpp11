...
#define QUANTITY_DEFINE_SCALING_LITERAL( sfx, dim, factor ) \
    constexpr quantity<dim, Rep> operator "" _ ## sfx(unsigned long long x) \
    { \
        return quantity<dim, Rep>( detail::magnitude_tag, factor * static_cast<Rep>( x )); \
    } \
    constexpr quantity<dim, Rep> operator "" _ ## sfx(long double x) \
    { \
        return quantity<dim, Rep>( detail::magnitude_tag, factor * static_cast<Rep>( x ) ); \
    }
